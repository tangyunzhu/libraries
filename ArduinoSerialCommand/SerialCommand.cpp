...
void SerialCommand::readSerial() 
{
	...
	#else
	while ((usingSoftwareSerial==0 && uart.available() > 0) || (usingSoftwareSerial==1 && SoftSerial->available() > 0) )
	#endif
	{
		int i; 
		boolean matched; 
		if (usingSoftwareSerial==0) {
			...
			inChar=uart.read();   // Read single available character, there may be more waiting
		} else {
			#ifndef SERIALCOMMAND_HARDWAREONLY
			// SoftwareSerial port
		...
		uart.print(inChar);   // Echo back to serial stream
		#endif
		if (inChar==term) {     // Check for the terminator (default '\r') meaning end of command
			...
			for (i=0; i<numCommand; i++) {
				#ifdef SERIALCOMMANDDEBUG
				uart.print("Comparing ["); 
				uart.print(token); 
				uart.print("] to [");
				uart.print(CommandList[i].command);
				uart.println("]");
				#endif
...
void SerialCommand::addCommand(const char *command, void (*function)())
{
	if (numCommand < MAXSERIALCOMMANDS) {
		#ifdef SERIALCOMMANDDEBUG
		uart.print(numCommand); 
		uart.print("-"); 
		uart.print("Adding command for "); 
		...
		// the command
		#ifdef SERIALCOMMANDDEBUG
		uart.println("Too many handlers - recompile changing MAXSERIALCOMMANDS"); 
		#endif 
	}
}
