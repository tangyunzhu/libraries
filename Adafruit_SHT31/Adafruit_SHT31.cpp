...
uint16_t Adafruit_SHT31::readStatus(void) {
  ...
  wireSHT.requestFromSHT(_i2caddr, (uint8_t)3);
  uint16_t stat = wireSHT.readSHT();
  stat <<= 8;
  stat |= wireSHT.readSHT();
...
boolean Adafruit_SHT31::readTempHum(void) {
  ...
  delay(500);
  wireSHT.requestFromSHT(_i2caddr, (uint8_t)6);
 /*  if (wireSHT.available() != 6) 
    return false; */
    delay(100);
  for (uint8_t i=0; i<6; i++) {
	delay(10);
    readbuffer[i] = wireSHT.readSHT();
...
void Adafruit_SHT31::writeCommand(uint16_t cmd) {
  wireSHT.beginTransmissionSHT(_i2caddr);
  wireSHT.writeSHT(cmd >> 8);
  wireSHT.writeSHT(cmd & 0xFF);
  wireSHT.endTransmissionSHT();  
}
